...
ImageValINT readImageFit2(string nombreImagen){
	...
	image.readAllKeys();
	image.read(contents);
    cout<< "datos de imagenhhhhhhh"<< image<<endl;
...
void normalicer(ImageValDouble& GTmp,ImageValDouble& pixCnt2,int dimX,int dimY){
...
ImageValDouble flatfield(ImageValDouble& val,const ImageValShort& tmp,int dimX,int dimY ){
  ...
  */
//ImageValDouble getConst(vector<ImageValInt>& data, const ImageValShort& tmp, ImageValDouble& pixCnt, const int disp[9][2],int dimX,int dimY) {
ImageValDouble getConst(vector<ImageValInt>& data, const ImageValShort& tmp, ImageValDouble& pixCnt, vector<vector<int>> disp,int dimX,int dimY) {
	...
	//	cout << "minimo maximo dat  "<< iq <<"   "<< dat[iq].min() <<"   "<< dat[iq].max() <<endl;
		...
		ImageValShort mskiq = (tmp & (unsigned short)(1 << iq)) / (1 << iq);

		for(unsigned short ir = 0; ir < iq; ir++) {

			// Obtencion de la mascara
			ImageValShort mskir = (tmp & (unsigned short)(1 << ir)) / (1 << ir);
			...
			mskiqROI = ROI(mskiq, -dx, -dy, dimX, dimY);//dx y dy en este

			ImageValDouble mskirROI;//(0.0,Alto*Ancho);
			mskirROI = ROI(mskir,dx, dy,dimX, dimY);

			ImageValDouble mskDouble = mskiqROI * mskirROI;

			//------------------------------
		//Calcula las regiones de interes de  las mascaras
			ImageValDouble datiqROI = ROI(dat[iq], -dx, -dy,dimX, dimY);
			ImageValDouble datirROI = ROI(dat[ir], dx, dy,dimX, dimY);
			...
			conJROI = diff;
			sumROI(con, conJROI, -dx, -dy,dimX, dimY);
			conIROI = - diff;
			sumROI(con, conIROI, dx, dy,dimX, dimY);
			...
			sumROI(pixCnt, pixCntJROI, -dx, -dy,dimX, dimY);
			pixCntIROI =  mskDouble;
			sumROI(pixCnt, pixCntIROI,  dx, dy,dimX, dimY);
...
//	con.copyTo(gainTmp);
	...
	for(unsigned short iq = 1; iq < no_of_image; iq++) {
		...
		for(unsigned short ir = 0; ir < iq; ir++) {
			...
			ImageValDouble mskiqROI = ROI(mskiq, -dx, -dy,dimX, dimY);
			ImageValDouble mskirROI = ROI(mskir, dx, dy,dimX, dimY);
			...
			ImageValDouble gainJROI=ROI(gain, -dx, -dy,dimX, dimY);
			ImageValDouble gainIROI=ROI(gain, dx,dy,dimX, dimY);

			// Modificar la ganancia temporal en base a la ganancia y la mascara
			gainTmpJROI = gainJROI*mskDouble;
			sumROI(gainTmp, gainTmpJROI, dx, dy,dimX, dimY);

			gainTmpIROI = gainIROI*mskDouble;
			sumROI(gainTmp, gainTmpIROI, -dx,-dy,dimX, dimY);
...
ImageValDouble iterate(const ImageValDouble& con, \
	...
	ImageValDouble flat=flatfield(gain,tmp, dimX,dimY );
