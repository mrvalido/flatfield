...
//         -38         140
	...
	//test four sets of 9 each displacement image
	...
	//Test for randon 9 and 16 images
	...
	for(unsigned int i = 1; i < numero; i++) {
		...
		//imageName[11] = 48 + i;//"./imF03/im0X.fits";
		//imageName[10] = 48 + i;//"./imF1/im0X.fits" y ./imF2/im0X.fits" ;
		//imageName[17] = 48 + i;//for ./Test_SolarC/im0X.fits" set

		//nombreImagen = imageName;

		nombreImagen = argv[1]+dir[i];
//		if (i<= 9) {
//			imageName[9]=48;
//		}
//		else {
...
//		nombreImagen = imageName;
		cout << "Image Name.....=" << nombreImagen	 << endl<<endl;


		datacube.push_back(readImageFit(nombreImagen,dimX,dimY));
//		unsigned int min,max;
//		//image=readImageFit(nombreImagen);
//		min=datacube[i-1].min();
//		max=datacube[i-1].max();

//		cout << "Dim x= "  	 << dimX << "  Dim Y="  	 << dimY << endl;
//
//		ImageValChar pixxx=escalado8(datacube[i-1]);
//		pinta2(pixxx,dimX,dimY,i-1);
//		waitKey(0);
	}

	cout << "Data cube created....."<< endl<<endl;
	ImageValShort tmp (dimX*dimY);//initial 16 bit mask
	cout << "SALI.333...y numero....."<<numero<<"...."<< endl;
	...
	//con = getConst(datacube, tmp, pixCnt, disp,dimX,dimY);
	con = getConst(datacube, tmp, pixCnt,disp,dimX,dimY);

	ImageValDouble gain(0.0,dimX*dimY);
	gain=con;
	normalicer(gain, pixCnt,dimX,dimY);

	//ImageValDouble flat = iterate(con, gain, tmp, pixCnt,disp, LOOPS,dimX,dimY);
	ImageValDouble flat = iterate(con, gain, tmp, pixCnt,disp, LOOPS,dimX,dimY);
	...
	cout << "Pintando Termino constante....."<<"...."<< endl<< endl;
	pinta2(pixx,dimX,dimY,1);
	int t2=writeImage(con,"con.fits",DOUBLE_IMG,dimX,dimY);
	waitKey(0);

	pixx=escalado8(gain);
	cout << "Pintando Termino Ganacia....."<<"...."<< endl<< endl;
	pinta2(pixx,dimX,dimY,2);
	waitKey(0);
	ImageValChar pix=escalado8(flat);
	cout << "Pintando Termino Flat Field....."<<"...."<< endl<< endl;
	pinta2(pix,dimX, dimY,3);

	waitKey(0);

#endif
	cout << "Writing flat Field....."<<"...."<< endl;
	string a;
	a=argv[1];
	a[a.size()-1]='.';
	a[0]='F';
	...
	string destino;
	cout << "argv   "<<destino<< endl<< endl;
	destino="F"+a+"fits";
	cout << "Pintando Termino Flat Field....."<<destino<< endl<< endl;
	int t=writeImage(flat,destino,DOUBLE_IMG,dimX,dimY);
	return 0;
}
